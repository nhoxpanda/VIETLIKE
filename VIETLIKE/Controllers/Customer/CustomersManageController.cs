    ...
    public class CustomersManageController : BaseController
    {
        ...
        public ActionResult ExcelSample()
        {
            try
            {
                ...
                using (var stream = new MemoryStream())
                {
                    string templateFile = System.Web.HttpContext.Current.Server.MapPath("~\\Upload\\ImportExport\\Import_KhachHangVietLike.xlsx");
                    ExportExcelTemplateCustomer(stream, templateFile, header);
                    bytes = stream.ToArray();
                }

                string fileName = "Mau-import-khach-hang-Vietlike.xlsx";
