...
namespace VIETLIKE.Controllers.Visa
{
    public class ListVisaManageController : BaseController
    {
        ...
        public ActionResult ExcelSample()
        {
            try
            {
                ...
                using (var stream = new MemoryStream())
                {
                    string templateFile = System.Web.HttpContext.Current.Server.MapPath("~\\Upload\\ImportExport\\Import_VisaVietLike.xlsx");
                    ExportExcelTemplateVisa(stream, templateFile, header);
                    bytes = stream.ToArray();
                }

                string fileName = "Mau-import-visa-Vietlike.xlsx";
